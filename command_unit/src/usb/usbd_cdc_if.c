...
static int8_t command_unit_Init     (void);
static int8_t command_unit_DeInit   (void);
static int8_t command_unit_Control  (uint8_t cmd, uint8_t* pbuf, uint16_t length);
static int8_t command_unit_Receive  (uint8_t* pbuf, uint32_t *Len);

USBD_CDC_ItfTypeDef USBD_CDC_command_unit_fops = 
{
  command_unit_Init,
  command_unit_DeInit,
  command_unit_Control,
  command_unit_Receive
};
...
/**
  ...
  * @retval Result of the operation: USBD_OK if all operations are OK else USBD_FAIL
  */
static int8_t command_unit_Receive(uint8_t* Buf, uint32_t *Len)
{
